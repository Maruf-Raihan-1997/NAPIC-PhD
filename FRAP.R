# -----------------------------
# FRAP Data
# -----------------------------
df_frap <- data.frame(
  Sample = c("Undigested", "DW-Water", "DW-Banana", "DW-Bread"),
  Mean   = c(30.20, 69.80, 45.30, 70.66),
  SD     = c(4.80, 1.00, 3.70, 9.30),
  CV     = c(15.80, 1.50, 8.20, 13.17),
  Tukey  = c("a", "c", "b", "c")
)

df_frap$Sample <- factor(df_frap$Sample,
                         levels = c("Undigested", "DW-Water", "DW-Banana", "DW-Bread"))

library(ggplot2)

p_frap <- ggplot(df_frap, aes(x = Sample, y = Mean, fill = Sample)) +
  geom_col(width = 0.7) +
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD),
                width = 0.2, linewidth = 1) +
  geom_text(aes(label = Tukey, y = Mean + SD + 8), size = 7) +
  
  scale_fill_manual(
    values = c("Undigested" = "darkgreen",
               "DW-Water" = "steelblue",
               "DW-Banana" = "forestgreen",
               "DW-Bread" = "saddlebrown"),
    name = "Sample Legend",
    guide = guide_legend(override.aes = list(shape = NA))
  ) +
  
  geom_point(aes(shape = Tukey), alpha = 0) +
  scale_shape_manual(
    name = "Tukey Groups",
    values = c("a" = 16, "b" = 16, "c" = 16),
    labels = c("a = Undigested",
               "b = DW-Banana",
               "c = DW-Water, DW-Bread")
  ) +
  
  guides(
    fill = guide_legend(order = 1),
    shape = guide_legend(order = 2, override.aes = list(alpha = 1))
  ) +
  
  theme_bw(base_size = 14) +
  theme(
    legend.position = "right",
    legend.box = "vertical",
    legend.spacing.y = unit(0.3, "cm"),
    legend.margin = margin(0, 0, 0, 0),
    legend.title = element_text(size = 13, face = "bold"),
    legend.text = element_text(size = 11),
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    axis.text.x = element_text(size = 14, face = "bold"),
    axis.text.y = element_text(size = 14, face = "bold"),
    axis.title.x = element_text(size = 16, face = "bold"),
    axis.title.y = element_text(size = 16, face = "bold"),
    axis.line = element_line(colour = "black", linewidth = 1.2),
    axis.ticks = element_line(colour = "black", linewidth = 1.2)
  ) +
  labs(
    title = "In Vitro Digestion Increased the Antioxidant Capacity (FRAP) of Duckweed",
    x = "Food Vehicle",
    y = "FRAP (µmol TE/g dry weight)"
  ) +
  annotate("segment", x = 1, xend = 2, y = 95, yend = 95, linewidth = 0.8) +
  annotate("text", x = 1.5, y = 100, label = "****", size = 6) +


  annotate("segment", x = 2, xend = 3, y = 88, yend = 88, linewidth = 0.8) +
  annotate("text", x = 2.5, y = 92, label = "**", size = 6) +
  annotate("segment", x = 3, xend = 4, y = 84, yend = 84, linewidth = 0.8) +
  annotate("text", x = 3.5, y = 88, label = "**", size = 6) +
  annotate("segment", x = 2, xend = 4, y = 120, yend = 120, linewidth = 0.8) +
  annotate("text", x = 3.0, y = 125, label = "ns", size = 5) +
  annotate("text", x = 1, y = 132,
           label = "**** p < 0.0001   |   ** p < 0.01   |   ns = not significant",
           hjust = 0, size = 4.5)

print(p_frap)